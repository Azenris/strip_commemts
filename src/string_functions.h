...
#ifndef _HG_STRING_FUNCTIONS
#define _HG_STRING_FUNCTIONS
...
/// @return Bytes
[[nodiscard]] inline constexpr u64 string_utf8_bytes( const char *str )
{
	assert( str );
...
{
	static_assert( destSize >= 1, "\"string_utf8_copy\" failed. Invalid destSize Size." );
...
constexpr u32 string_utf8_codepoint( const char **pStr )
{
	const char *str = *pStr;
	...
	log_warning( "Function \"string_utf8_codepoint\" failed. UTF-8 5 & 6 byte characters not supported." );
	return 0;
}

u32 string_utf8_codepoint( const char *str, u32 *pSize );

[[nodiscard]] inline u32 string_utf8_peek_codepoint( const char *str )
{
	u32 size;
	return string_utf8_codepoint( str, &size );
}
...
[[nodiscard]] inline char *string_utf8_get_base_filename( const char *str, Allocator *allocator )
{
...
/// @return	appended bytes - NOT including the null terminator
u64 string_utf8_append( char *destination, u64 destSize, const char *append, ... );

/// @return	appended bytes - NOT including the null terminator
u64 string_utf8_append_args( char *destination, u64 destSize, const char *append, va_list args );
...
u64 string_utf8_append( char( &destination )[ destSize ], const char *append, ... )
{
	...
	u64 result = string_utf8_append_args( destination, destSize, append, args );
	va_end( args );
	return result;
}
...
#endif // _HG_STRING_FUNCTIONS

#if defined( STRING_FUNCTIONS_IMPLEMENTATION )
...
i32 string_utf8_format_args( char *destination, u64 destSize, const char *format, va_list args )
{
	...
	if ( result >= destSize )
	{
		log_warning( "string_utf8_format destination buffer too small. [ %d / %llu ]", result, destSize );
		if ( result == destSize )
			log_warning( "Make sure there is enough room for the ending null terminator" );
		destination[ 0 ] = '\0';
		return -1;
	}
...
[[nodiscard]] utf8Character string_utf8_encode( u32 codepoint )
{
	...
	// NOTE : 5 & 6 byte characters not supported
	log_warning( "Function \"string_utf8_from_codepoint\" failed. UTF-8 5 & 6 byte characters not supported." );
	character.data[ 0 ] = '\0';
	return character;
}
...
u64 string_utf8_append_args( char *destination, u64 destSize, const char *append, va_list args )
{
	u64 p = string_utf8_bytes( destination ) - 1; // -1 to overwrite the null terminator
	return string_utf8_format_args( destination + p, destSize - p, append, args );
}
